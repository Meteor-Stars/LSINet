...
class EIBLayer(nn.Module):
    def __init__(self, PatchNum, PatchEmbedDim, dropout, args=None):
        ...
        self.self_attn = _MultiheadAttention(args.d_model, args.n_heads, args.d_model, args.d_model, attn_dropout=0,
                                     proj_dropout=0, res_attention=False)
        self.sparse_k = int(PatchNum * PatchNum* 0.3)  # 0.95 0.3 0.2

    def Binary_Masks_Fitting(self,X,dropout_rate=0.9,mode='train'):
        if mode=='train':
            binary_mask_matrix = (torch.randn(X.shape) > dropout_rate).float().to(X.device)
            return binary_mask_matrix * X
        elif mode=='test':
            """
            Sampling the well-fitted binary_mask_matrix from the Bernoulli distribution
            """
            if self.args.sample_num!=0:
                samples_all=[]
                for _ in range(self.args.sample_num):
                    binary_mask_matrix = (torch.randn(X.shape) > dropout_rate).float().to(X.device)
                    samples_all.append(binary_mask_matrix * X)
                return samples_all
            else:
                binary_mask_matrix = (torch.randn(X.shape) > dropout_rate).float().to(X.device)
                return binary_mask_matrix * X
    ...
    def forward(self, x_or):
        ...
        elif self.args.ESIMechanism:
            binary_mask_matrix=torch.full((self.PatchNum,self.PatchNum),1).to(x_or.device)
            if self.args.test:
                binary_mask_matrix=self.Binary_Masks_Fitting(binary_mask_matrix,dropout_rate=0.85,mode='test') #0.95 0.65 0.85
            else:
                binary_mask_matrix=self.Binary_Masks_Fitting(binary_mask_matrix,dropout_rate=0.85,mode='train')#0.95 0.65 0.85
            if self.args.test and self.args.sample_num!=0:
                x_p_samples_all=[]
                for i in range(self.args.sample_num):
                    x_p = torch.matmul(binary_mask_matrix[i].unsqueeze(0), x_or)  # .transpose(1, 2)
                    x_p_samples_all.append(x_p.unsqueeze(0))
                x_p=torch.mean(torch.cat(x_p_samples_all,dim=0),dim=0)
            else:
                x_p = torch.matmul(binary_mask_matrix.unsqueeze(0), x_or)#.transpose(1, 2)
